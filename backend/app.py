...
def buy_now():
    ...
    purchase_query = """
        ...
        VALUES (?, ?, ?, ?)
    """
    c.execute(purchase_query, (productId, buyer_email,
              buy_now_price, currentTime))

    # Send confirmation emails
    send_purchase_confirmation(
        buyer_email, seller_email, product_name, buy_now_price)
        ...
        if current_bidder != highest_auto_bidder[0]:
            ...
            if new_bid <= highest_auto_bidder[1] and new_bid > current_bid_amount:
                ...
                insert_query = """
                    ...
                    VALUES (?, ?, ?, ?)
                """
                c.execute(insert_query, (product_id,
                          highest_auto_bidder[0], new_bid, currentTime))
                conn.commit()
