...
describe('ProductDetails Component', () => {
    const mockProductData = {
        data: {
            ...
            product: [
                [1, 'Test Product', 'https://example.com/image.jpg', 'Seller1', 100, '2023-11-01', 10, '2023-12-01', 'Sample description'],
            ],
        },
    };
    ...
    test('displays "No bids so far" when there are no bids', async () => {
        axios.post.mockResolvedValueOnce({ data: { bids: [], product: mockProductData.data.product } });
    ...
    test('displays bids when available', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Bidder: User1 user1@example.com')).toBeInTheDocument();
            expect(screen.getByText('Bid amount: $120')).toBeInTheDocument();
            expect(screen.getByText('Bidder: User2 user2@example.com')).toBeInTheDocument();
    ...
    test('toggles AddBid component visibility on button click', async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.getByPlaceholderText('Enter your bid amount here')).toBeInTheDocument();
        });
