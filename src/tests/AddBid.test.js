...
describe('AddBid Component', () => {
    ...
    test('renders AddBid component correctly', () => {
        render(<AddBid productId={mockProductId} sellerEmail={mockSellerEmail} />);
        
        expect(screen.getByText(`Add a bid for product ${mockProductId}`)).toBeInTheDocument();
    ...
    test('displays error toast if user tries to bid on own product', () => {
        ...
        fireEvent.change(screen.getByPlaceholderText('Enter your bid amount here'), { target: { value: mockBidAmount } });
